...
def export_products_csv(product_ids):
    products = Product.objects.filter(id__in=product_ids).prefetch_related('skus__template').select_related('country', 'category')
    output = io.StringIO()
    ...
    for p in products:
        ...
        writer.writerow([p.id, p.title, p.description, p.size_info, p.country.name, p.category.name, sku_colors, p.get_status_display(), p.created_at.strftime('%Y-%m-%d %H:%M')])
    return output.getvalue()


def export_products_zip(product_ids):
    products = Product.objects.filter(id__in=product_ids).prefetch_related('skus').select_related('country', 'category')
    buf = io.BytesIO()
